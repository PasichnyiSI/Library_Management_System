...
- `routers/`: Частина програми, яка містить всі API-методи.
  ...
  - `borrowers.py`: Містить маршрути, які обробляють запити для управління читачами в системі.
...
```
Ці змінні використовуються для налаштування підключення до бази даних та параметрів автентифікації.
...
Данна бібліотична система для валідації запитів використовує схеми Pydantic, які забезпечують перевірку коректності вхідних даних для кожного API-методу. Валідація допомагає запобігти неправильним або неповним даним, що можуть бути передані в запитах.
